...
public class GeneralMotorTest extends OpMode {
    ...
    public void loop() {
        ...
        if(motors[motorTog.currentState()] != null){
            powerTog.changeState(gamepad1.dpad_up || gamepad2.dpad_up, gamepad2.dpad_up||gamepad2.dpad_down);
            onOffTog.changeState(gamepad1.y || gamepad2.y);
            if(onOffTog.currentState() == 0){
                motors[motorTog.currentState()].setPower(0);
            } else {
                motors[motorTog.currentState()].setPower((powerTog.currentState()-101)/100);
            }
            ...
            telemetry.addData("Velocity in RPM",motors[motorTog.currentState()].getVelocity(AngleUnit.DEGREES)/(360*60)); //degrees per second to rpm
